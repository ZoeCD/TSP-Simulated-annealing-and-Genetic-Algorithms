...
def animateGenetic(history, points, speed):
    ...
    def init():
        ...
        city_information = pd.read_csv("Data/cities.csv")
        x = [points[i][0] for i in history[0]]
        y = [points[i][1] for i in history[0]]
        plt.plot(x, y, 'co')
        for i_x, i_y in zip(x, y):
            city = city_information[(i_x == city_information["x"]) & (
                i_y == city_information["y"])]
            plt.text(i_x, i_y, '({})'.format(city.values[0][0]))
